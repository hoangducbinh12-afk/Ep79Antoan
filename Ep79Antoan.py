...
def get_matrix_df_v13(db, last_full_str):
    if not db or not last_full_str: 
        return pd.DataFrame(columns=["Số", "Điểm", "Rank"]) # Trả về khung trống để tránh lỗi KeyError
...
def get_low_5_realtime(df_rank):
    if df_rank.empty or "Số" not in df_rank.columns: 
        return ["?"]*5
...
# --- 3. GIAO DIỆN ---
st.set_page_config(layout="wide", page_title="Matrix V13.86 Fixed")
st.title("🛡️ Matrix V13.86 - Bottom Scan (Stable)")

# Khởi tạo session state để tránh lỗi khi chưa nạp file
...
with st.sidebar:
    ...
    up_img = st.file_uploader("Upload ảnh kết quả", type=['jpg','png','jpeg'])
    ...
    raw_in = st.text_area("27 giải loto:", value=st.session_state['raw_input'], height=100)
    ...
    if st.button("🔥 PHÂN TÍCH & LƯU", type="primary", use_container_width=True):
        ...
        if len(raw_list) >= 27 and gdb_now and st.session_state['db']:
            ...
            st.error("Chưa nạp file .json kìa mày!")

# --- HIỂN THỊ CHÍNH ---
if not st.session_state['db']:
    st.warning("👈 Mày cần nạp file .json ở bên trái trước đã!")
else:
    # Tính Rank
    df_current_rank = get_matrix_df_v13(st.session_state['db'], st.session_state['last_full_str'])
    
    if not df_current_rank.empty:
        low_5 = get_low_5_realtime(df_current_rank)
        
        # Ô THÔNG SỐ
        st.markdown(f"### 🔮 5 Chạm Đáy Biến Thiên: `{','.join(low_5)}`")
        ...
        if st.session_state['history']:
            ...
            def check_at(row):
                g = str(row['GĐB'])[-2:]
                is_hit = any(d in g for d in low_5)
                is_kep = g[0] == g[1] if len(g)==2 else False
                return "A" if (is_hit or is_kep) else "T"
            df_h['Kết quả'] = df_h.apply(check_at, axis=1)
            st.dataframe(df_h.reindex(columns=['STT', 'GĐB', 'Kết quả']).head(15), use_container_width=True)
        ...
        st.success(f"🎯 Dàn 80 số tối ưu cho kỳ sau (Dựa trên chạm {','.join(low_5)})")
